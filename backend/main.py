...
async def health():
    ...
    from backend.services.data_fetchers import VERIFIED_PERMIT_CITIES, TOTAL_PERMIT_CITIES
    mcp_tools = await elastic.list_mcp_tools() if elastic.mcp_available else []
    data_tiers = ["nyc"] + sorted(_PERMIT_CONFIGS.keys())
    return {
        "status": "ok",
        "service": "blueprint",
        ...
        "permit_cities": TOTAL_PERMIT_CITIES,
        "verified_permit_cities": VERIFIED_PERMIT_CITIES,
        "permit_coverage": (
            f"{VERIFIED_PERMIT_CITIES} US cities with schema-mapped Socrata permit feeds; "
            f"{TOTAL_PERMIT_CITIES} portals wired total. Every US address gets full "
            f"FEMA + USGS + EPA + OSM analysis regardless of permit coverage."
        ),
        ...
        "elastic_capabilities": [
            "hybrid_retrieval",
            "rrf_hybrid_retrieval",
            "elser_semantic_search",
            "semantic_reranking",
            "esql_cross_reference",
            "esql_semantic_rerank",
            "geo_distance_cross_reference",
            "aggregation_analytics",
            "significant_terms_intelligence",
            "percolator_risk_alerts",
            "memory_layer_writeback",
            "watchlist_24h_monitoring",
            "auto_watch_high_risk",
            "agent_builder_custom_tools",
            "mcp_tool_execution",
        ],
        "elastic_agent_builder_tools": elastic.agent_builder_tools,
        "adk_mcp_toolset": (
            "attached to SynthesisAgent (MCP connected)"
            if elastic.mcp_available else
            "configured, activates when MCP connects"
            if (settings.ELASTIC_MCP_URL and settings.ELASTIC_API_KEY) else
            "disabled (ELASTIC_MCP_URL not set)"
        ),
    }
    ...
    return {
        "score_bands": [
            ...
            },
            {
                ...
                "meaning": "Serious compounding risk factors identified — unresolved permits, hazard zone exposure, environmental proximity, or title anomalies. Demand seller resolution of flagged items, commission independent assessments, and negotiate a price reduction of 8–15%."
            },
            {
                "range": "81–100", "label": "CRITICAL", "color": "#ef4444",
                "headline": "Serious risk, specialist review required",
                "meaning": "Severe or multiple compounding hazards. Consult a real estate attorney, licensed inspector, and relevant specialist (flood plain manager, environmental engineer) before proceeding. Do not make an offer without professional due diligence."
            },
        ],
        "risk_factors": [
...
async def similar_properties(address_hash: str):
    ...
    from backend.services.elastic_client import IDX_REPORTS
    if not elastic.available:
        return {"count": 0, "properties": [], "current_risk_level": "UNKNOWN", "method": "unavailable"}
    try:
        doc = await elastic._es.get(index=IDX_REPORTS, id=address_hash)
        current = doc["_source"]
    except Exception:
        return {"count": 0, "properties": [], "current_risk_level": "UNKNOWN", "method": "not_found"}

    risk_level = current.get("risk_level", "HIGH")
    lat, lng   = current.get("lat"), current.get("lng")

    # 1, true geographic cross-reference via geo_distance (nearest first)
    method = "geo_distance"
    props  = []
    if lat is not None and lng is not None:
        props = await elastic.geo_search_nearby(lat, lng, radius_km=50, exclude_hash=address_hash, size=6)

    # 2, fall back to same-risk-band via ES|QL when no geo neighbours exist yet
    if not props:
        method = "risk_level_esql"
        esql = (
            f'FROM {IDX_REPORTS} '
        ...
        props = await elastic.esql(esql)

    # significant_terms: which flags distinguish this risk band corpus-wide
    significant = await elastic.significant_flags(risk_level)
...
async def elastic_status():
    """Live, fully data-driven Elastic capability dashboard.
    The frontend renders the entire 'Elastic Intelligence' panel from this payload —
    capability cards, index counts, MCP tools, ES|QL queries, nothing hardcoded."""
    from backend.services.elastic_client import (
        IDX_CASES, IDX_EVENTS, IDX_REPORTS, IDX_SHARED, IDX_WATCHED, IDX_ALERTS,
        ELSER_INFERENCE_ID, RERANK_INFERENCE_ID,
    )
    es   = elastic.available
    mcp  = elastic.mcp_available
    perc = elastic.percolator_ready
    abt  = len(elastic.agent_builder_tools) > 0
    idx_stats = await elastic.index_stats() if es else {}
    mcp_tools = await elastic.list_mcp_tools() if mcp else []

    # Each capability is a self-describing card. `active` reflects live cluster state.
    capabilities = [
        {"id": "elser_semantic", "category": "Retrieval", "active": es,
         "label": "ELSER semantic search",
         "detail": f"Sparse-vector semantic_text retrieval via {ELSER_INFERENCE_ID} over heterogeneous property records."},
        {"id": "rrf_hybrid", "category": "Retrieval", "active": es,
         "label": "RRF hybrid retriever",
         "detail": "Reciprocal Rank Fusion blends BM25 lexical and ELSER semantic rankings into one result set."},
        {"id": "semantic_reranker", "category": "Retrieval", "active": es,
         "label": "Semantic reranker",
         "detail": f"text_similarity_reranker ({RERANK_INFERENCE_ID}) reorders hits by risk relevance."},
        {"id": "esql", "category": "Analytics", "active": es,
         "label": "ES|QL cross-reference",
         "detail": "Five ES|QL queries per analysis: distribution, permit-sale timing, high-confidence, RERANK, flip-fraud."},
        {"id": "geo_distance", "category": "Analytics", "active": es,
         "label": "Geo-distance cross-reference",
         "detail": "geo_point + geo_distance surface previously-analysed properties within 50 km, nearest first."},
        {"id": "aggregations", "category": "Analytics", "active": es,
         "label": "Aggregation analytics",
         "detail": "terms · stats · percentiles · date_histogram · cardinality power live market intelligence."},
        {"id": "significant_terms", "category": "Analytics", "active": es,
         "label": "Significant-terms intelligence",
         "detail": "significant_terms surfaces risk flags statistically over-represented in each risk band."},
        {"id": "percolator", "category": "Proactive", "active": perc,
         "label": "Percolator risk alerts",
         "detail": "Saved risk-profile queries; every new property is reverse-searched to fire proactive alerts."},
        {"id": "memory_writeback", "category": "Memory", "active": es,
         "label": "Memory-layer write-back",
         "detail": "Every agent finding is persisted and becomes searchable context for future analyses."},
        {"id": "agent_builder_tools", "category": "Agent Builder", "active": abt,
         "label": "Custom Agent Builder tools",
         "detail": "Custom ES|QL tools provisioned in Agent Builder and exposed to the Gemini agent over MCP."},
        {"id": "mcp_execution", "category": "Agent Builder", "active": mcp,
         "label": "Agent Builder MCP",
         "detail": "platform.core.search / platform.core.esql invoked over Streamable HTTP MCP transport."},
    ]

    return {
        "elastic_connected": es,
        "elastic_mcp": "connected" if mcp else "unavailable (direct SDK active)",
        "elastic_mcp_endpoint": settings.ELASTIC_MCP_URL or "not configured",
        "elastic_mcp_tools": mcp_tools,
        "agent_builder_tools": elastic.agent_builder_tools,
        "percolator_ready": perc,
        "index_document_counts": idx_stats,
        "capabilities": capabilities,
        "indices": {
            IDX_CASES:   "Property case files, geocoded addresses + geo_point + pipeline status",
            IDX_EVENTS:  "Property events, permits, deeds, climate, neighborhood (ELSER semantic_text)",
            IDX_REPORTS: "Synthesised reports, risk scores, escape plans, debate verdicts, geo_point",
            IDX_SHARED:  "Share links, public report access with expiry",
            IDX_WATCHED: "Watchlist, monitored properties re-analysed every 24h",
            IDX_ALERTS:  "Percolator, saved risk-profile queries for proactive alerting",
        },
        "elser_inference_endpoint": ELSER_INFERENCE_ID,
        "reranking_inference_endpoint": RERANK_INFERENCE_ID,
        "retrieval_strategies": [
            "Elastic Agent Builder MCP hybrid search",
            "RRF hybrid retriever (BM25 ⊕ ELSER)",
            "Semantic reranker (text_similarity_reranker)",
            "BM25 keyword (graceful fallback)",
        ],
        "esql_queries": [
            ...
            "Flip-fraud detection (rapid repeated deed transfers)",
        ],
    }


@app.get("/api/elastic/insights")
async def elastic_insights():
    """Live cross-property market intelligence, powered entirely by Elastic aggregations.
    Demonstrates the memory layer compounding: each analysis makes these insights richer."""
    if not elastic.available:
        return {"available": False, "insights": {}, "significant_terms_by_band": {}}
    insights = await elastic.market_insights()
    sig: dict = {}
    for level in ("CRITICAL", "HIGH", "MEDIUM", "LOW"):
        flags = await elastic.significant_flags(level)
        if flags:
            sig[level] = flags
    return {"available": True, "insights": insights, "significant_terms_by_band": sig}
...
async def coverage():
    ...
    from backend.services.data_fetchers import (
        _PERMIT_CONFIGS, VERIFIED_PERMIT_CITIES, TOTAL_PERMIT_CITIES,
    )
    cities = [
        {
            "tier": tier,
    ...
    # Add NYC which has its own dedicated fetcher (real dataset ids → verified)
    cities.insert(0, {
        "tier": "nyc",
    ...
    return {
        "total_permit_cities": TOTAL_PERMIT_CITIES,
        "verified_permit_cities": VERIFIED_PERMIT_CITIES,
